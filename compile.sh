...
TEST_NAME="$(uname)" # MINGW64_NT-10.0-26200
...
function regen_compiledb() {
	"${MAKE}" --no-print-directory -C "${BUILD_DIR}" clean >/dev/null
	test $? -ne 0 && exit 1

	"${MAKE}" --no-print-directory -C "${BUILD_DIR}" -n >"${BUILD_DIR}"/build.log
	#
...
function regen_clangd_config() {
	...
	"${MAKE}" --no-print-directory -C "${BUILD_DIR}" dump_compile_info
...
---
If:                               
  PathMatch: .*\.(hpp|hxx|cc|cpp|CPP|cxx)    
Diagnostics:      
...
CompileFlags:           
  ...
  Add:
    ...
    - "-xc++"   
EOF
	cat "${BUILD_DIR}"/cppflags_strip_lines | while read -r line; do
...
function build_target() {
	"${MAKE}" --no-print-directory -C "${BUILD_DIR}" -j1
	test $? -ne 0 && exit 1
}
...
function rebuild_target() {
	"${MAKE}" --no-print-directory -C "${BUILD_DIR}" clean
	test $? -ne 0 && exit 1
	"${MAKE}" --no-print-directory -C "${BUILD_DIR}" -j"$(nproc)"
	test $? -ne 0 && exit 1
}

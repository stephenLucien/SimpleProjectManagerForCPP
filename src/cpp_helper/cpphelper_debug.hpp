...
#define SNPRINTF_STRUCTURE_FIELD(buf, bufsz, offset, prefix, surfix, pst, field)                                                                  \
    ({                                                                                                                                            \
        ...
        { /* char or signed char */                                                                                                               \
            ...
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%hhd(0x%02X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix); \
        } else if (typeid(((pst)->field)) == typeid(uint8_t))                                                                                     \
        { /* unsigned char */                                                                                                                     \
            auto tmp_ptr = ((const uint8_t*)(&((pst)->field)));                                                                                   \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%hhu(0x%02X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix); \
        } else if (typeid(((pst)->field)) == typeid(int16_t))                                                                                     \
        { /* signed short */                                                                                                                      \
            auto tmp_ptr = ((const int16_t*)(&((pst)->field)));                                                                                   \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%hd(0x%04X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix);  \
        } else if (typeid(((pst)->field)) == typeid(uint16_t))                                                                                    \
        { /* unsigned short */                                                                                                                    \
            auto tmp_ptr = ((const uint16_t*)(&((pst)->field)));                                                                                  \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%hu(0x%04X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix);  \
        } else if (typeid(((pst)->field)) == typeid(int32_t))                                                                                     \
        {                                                                                                                                         \
            auto tmp_ptr = ((const int32_t*)(&((pst)->field)));                                                                                   \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%d(0x%08X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix);   \
        } else if (typeid(((pst)->field)) == typeid(uint32_t))                                                                                    \
        {                                                                                                                                         \
            auto tmp_ptr = ((const uint32_t*)(&((pst)->field)));                                                                                  \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%u(0x%08X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix);   \
        } else if (typeid(((pst)->field)) == typeid(int64_t))                                                                                     \
        {                                                                                                                                         \
            auto tmp_ptr = ((const int64_t*)(&((pst)->field)));                                                                                   \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%ld(0x%016X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix); \
        } else if (typeid(((pst)->field)) == typeid(uint64_t))                                                                                    \
        {                                                                                                                                         \
            auto tmp_ptr = ((const uint64_t*)(&((pst)->field)));                                                                                  \
            ret          = snprintf((buf) + (offset), (bufsz) - (offset), "%s" #field "=%lu(0x%016X)%s", prefix, (*tmp_ptr), (*tmp_ptr), surfix); \
